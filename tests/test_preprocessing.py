...
def test_target_variable():
    """Test: vérifier que la variable cible est définie"""
    from ml.config import TARGET
    
    assert TARGET == 'lap_duration'

def test_test_size_config():
    """Test: vérifier la config du split train/test"""
...
def test_cv_folds():
    """Test: vérifier le nombre de folds pour CV"""
...
def test_mlflow_config():
    """Test: vérifier la config MLflow"""
...
def test_gridsearch_params():
    """Test: vérifier les paramètres GridSearch"""
    ...
    # Vérifier que ce sont des dictionnaires avec des listes
    for model_name, params in GRIDSEARCH_PARAMS.items():
        assert isinstance(params, dict)

def test_features_groups():
    """Test: vérifier les groupes de features"""
...
def test_random_state():
    """Test: vérifier que le random state est fixé"""
